    ...
    def load_param(self, model_path):
        ...
        for i in param_dict:
            ...
            try:
                ...
                print("===========================ERROR=========================")
                print(
                    "shape do not match in i :{}: param_dict{} vs self.state_dict(){}".format(
                        i, param_dict[i].shape, self.state_dict()[i].shape
                    )
                )
...
class VisionTransformer_mask(nn.Module):
    ...
    def __init__(
        ...
        self.mask_embedding = nn.Parameter(
            torch.zeros(64, num_patches, embed_dim)
        )  # 【768】

        trunc_normal_(self.mask_embedding, std=0.02)
...
class TransReID(nn.Module):
    ...
    def __init__(
        ...
        trunc_normal_(self.cls_token, std=0.02)  # 0.01 better
        trunc_normal_(self.pos_embed, std=0.02)
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight, std=0.02)  # 0.01 bette # 0.01 betterr
            if isinstance(m, nn.Linear) and m.bias is not None:
                nn.init.constant_(m.bias, 0)
        elif isinstance(m, nn.LayerNorm):
            nn.init.constant_(m.bias, 0)
