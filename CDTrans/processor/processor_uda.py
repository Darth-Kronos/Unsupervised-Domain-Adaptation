...
def compute_knn_idx(
    ...
    if topk == 1:
        ...
        target_knnsim, target_knnidx = (
            target_knnsim[topk - 1, :],
            target_knnidx[topk - 1, :],
        )
...
def count_target_usage(
    ...
    if source_idxs is not None:
        ...
        logger.info(
            "source number usage: {}".format(len(source_unique_knnidx) / img_num1)
        )
    else:
...
def do_train_uda(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (imgs, vid, target_cam, target_view, file_name, idx) in enumerate(
            train_loader
        ):
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (img, vid, camid, camids, target_view, idex) in enumerate(
                    val_loader
                ):
                    with torch.no_grad():
                        img = img.to(device)
                ...
                if accuracy > best_model_mAP:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(
                            cfg.OUTPUT_DIR, cfg.MODEL.NAME + "_best_model.pth"
                        ),
                    )
                logger.info(
                    "Classify Domain Adapatation Validation Results - Epoch: {}".format(
                        epoch
                    )
                )
                logger.info(
                ...
                for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(
                    ...
                    with torch.no_grad():
                        ...
                        target_view = target_view.to(device)
                        feat, feat2 = model(
                            img, img, cam_label=camids, view_label=target_view
                        )
...
def do_inference_uda(cfg, model, val_loader, num_query):
    ...
    if cfg.MODEL.TASK_TYPE == "classify_DA":
        ...
        evaluator = R1_mAP_draw_figure(
            cfg, num_query, max_rank=50, feat_norm=True, reranking=cfg.TEST.RE_RANKING
        )
