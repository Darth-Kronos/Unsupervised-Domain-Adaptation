...
class build_uda_transformer(nn.Module):
    ...
    def forward(
        ...
        global_feat, global_feat2, global_feat3, cross_attn = self.base(
            x,
            x2,
            cam_label=cam_label,
        ...
        if return_logits:
            if self.ID_LOSS_TYPE in ("arcface", "cosface", "amsoftmax", "circle"):
                ...
                cls_score3 = (
                    self.classifier(feat3, label) if global_feat3 is not None else None
                )
            else:
                cls_score = self.classifier(feat) if global_feat is not None else None
                cls_score2 = self.classifier(feat2)
                cls_score3 = (
                    self.classifier(feat3) if global_feat3 is not None else None
                )
        ...
        if self.training or return_feat_prob:
            ...
            return (
                ...
                cross_attn,
            )  # source , target , source_target_fusion
...
def make_model(cfg, num_class, camera_num, view_num):
    if cfg.MODEL.NAME == "transformer":
        if cfg.MODEL.BLOCK_PATTERN == "3_branches":
            model = build_uda_transformer(
                num_class, camera_num, view_num, cfg, __factory_hh
            )
