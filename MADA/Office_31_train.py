...
root_path = train_func.Office_31_root_path
feas_dim = 2048
n_classes = 31
batch_size = 256
encoder_out_dim = 512
...
def train_to_csv(ds, dt, iterations=5, fea_type='Resnet50', n_Dtl=0.03):
    ...
    dataloader_ds, dataloader_dt = train_func.get_sd_td_with_labels_dataloader(
    ...
    for _ in range(iterations):
        ...
        mada_net = MADA.MADA_networks(in_dim=feas_dim, n_classes=n_classes, encoder_out_dim=encoder_out_dim).cuda()

        acc_tgt_best = MADA.training(
...
if __name__ == '__main__':
    ...
    train_to_csv(train_func.domain_a, train_func.domain_ad)
    train_to_csv(train_func.domain_a, train_func.domain_aw)

    train_to_csv(train_func.domain_d, train_func.domain_da)
    train_to_csv(train_func.domain_w, train_func.domain_wa)

    train_to_csv(train_func.domain_d, train_func.domain_dw)
    train_to_csv(train_func.domain_w, train_func.domain_wd)